...
if(!file_exists($входяший_bin)) {echo iconv("UTF-8", "CP866", "\n\rОтсутствует файл bin ".$входяший_txt); system("pause"); exit();}
...
//echo iconv("UTF-8", "CP866", "поехали");


//выдёргиваем маркеры текста
 $длина_заголовка = hexdec(bin2hex(substr($full_bin,4,4)));
$длина_текста_безФФ = hexdec(bin2hex(substr($full_bin,8,4)));

$ФФ_в_конце_текста =  round((ceil(($длина_заголовка+$длина_текста_безФФ)/16) - ($длина_заголовка+$длина_текста_безФФ)/16)*16) ;
$длина_текста = $длина_заголовка+$длина_текста_безФФ+$ФФ_в_конце_текста;


if(substr_count($full_bin,hexbin("3B2B78DA"))){ // если в файле есть LAST_UPDATE_DATE_TIME берём маркеры по другому адресу

$маркер_текста = substr($full_bin,$длина_текста+48,4);   // 58ECE6D7
$маркер_конца_текста = substr($full_bin,$длина_текста+56,4); //  7BE3FFC9

}
else
{
$маркер_текста = substr($full_bin,$длина_текста+24,4);   // 58ECE6D7
$маркер_конца_текста = substr($full_bin,$длина_текста+32,4); //  7BE3FFC9
}
//echo bin2hex($маркер_текста);
//определяем первое вхождение текста 58ECE6D7
$text_block_start = strpos($full_bin,$маркер_текста);

//чистим масив от лишних знаков, но оставляем пробел
...
$text_count =  substr_count($full_bin,$маркер_текста)-1;  // минус одина штука что в подвале
...
//$text_indexs = hexbin("9521F65800FFFFFF0CBA8B3F00FFFFFF");
...
$text_block_end = strpos($full_bin,$маркер_конца_текста);
